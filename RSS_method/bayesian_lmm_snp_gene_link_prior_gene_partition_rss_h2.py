...
def print_snp_gene_link_output_file(window_names, class_membership, window_info, ordered_genes, snp_gene_link_output_file, method_version):
	...
	for window_name in window_names:
		...
		for snp_index, snp_name in enumerate(window_rsids):
			...
			elif method_version == 'snp_gene_component':
...
class Bayesian_LMM_RSS_h2_inference(object):
	def __init__(self, window_names, window_info, n_gwas_individuals, ordered_genes, gene_categories, output_stem,inv_gamma_alpha=1e-10, inv_gamma_beta=1e-10, cross_gene_hyperparm=False,method_version='null_component'):
	...
	def fit(self, total_iterations=15000, burn_in_iterations=10000, update_resid_var_bool=True):
		...
		for itera in range(total_iterations):
			...
			t.write(','.join((self.inv_gamma_betas/self.inv_gamma_alpha).astype(str))+ '\n')
			t.flush()
	...
	def update_inv_gamma_beta_prior(self, beta0_tmp=0.0, alpha0_tmp=0.0):
		...
		valid_gamma_var = self.gamma_var[self.valid_gene_indices]
		valid_gene_categories = self.gene_categories[self.valid_gene_indices]

		for gc_iter in range(self.GC):
		
			gamma_b =beta0_tmp + np.sum(1.0/valid_gamma_var[valid_gene_categories==gc_iter])
			gamma_a = alpha0_tmp + len(valid_gamma_var[valid_gene_categories==gc_iter])*self.inv_gamma_alpha

			self.inv_gamma_betas[gc_iter] = np.random.gamma(gamma_a, scale=1/gamma_b,size=1)[0]
	...
	def update_gamma_var(self, v0=0.0, s_sq=0.0):
		...
		for window_name in self.window_names:
			...
			elif self.method_version == 'snp_gene_component':
		...
		param2 = 1.0/((tau_sq/2) + self.inv_gamma_betas[self.gene_categories]) # inv_gamma_beta == self.gene_gamma_scale)
	...
	def initialize_variables(self):
		...
		self.GC = len(np.unique(self.gene_categories))
		self.inv_gamma_betas = np.ones(self.GC)*self.inv_gamma_beta
