...
###########################
# Output root
tmp_output_root="/n/scratch/users/b/bes710/snp_gene_disease_links_ldsc_method/"
...
##############################
if false; then
sbatch process_LD_data.sh $hapmap3_rsid_file $baselineLD_anno_dir $kg_plink_dir $processed_ld_data_dir
fi
...
K_closest_genes="10"
if false; then
sbatch generate_snp_gene_annotations.sh $baselineLD_anno_dir $gene_annotation_file $K_closest_genes $snp_gene_annotation_dir $processed_ld_data_dir $preorganized_snp_gene_annotation_dir
fi
...
input_window_summary_file=${preorganized_snp_gene_annotation_dir}"window_LD_summary_with_snp_gene_anno_v2.txt"
gene_summary_file=${preorganized_snp_gene_annotation_dir}"gene_name_to_integer_mapping.txt"
output_stem=${learned_snp_gene_links_dir}"snp_gene_links_constrained_gs_"${trait_name}
sbatch run_snp_gene_disease_linking_w_constrained_gs.sh $trait_name $trait_file $input_window_summary_file ${gene_summary_file} $constrained_genes_geneset_file ${output_stem}
